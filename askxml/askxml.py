...
class AskXML:
    ...
    def synchronize(self):
        ...
        try:
            ...
            for root_tag, hierarchy in self._driver.table_hierarchy.items():
                root_tags_data = self._sync_cursor.execute("SELECT * FROM {from_table}".format(from_table=root_tag))
                self._synchronize_tags(root_tags_data.fetchall(), hierarchy, table_scope=root_tag)
        finally:
    ...
    def _serialize_properties(self, properties):
        ...
        if len(filtered_properties) > 0:
            return ' ' + ' '.join('{}="{}"'.format(name, val) for name, val in filtered_properties)
        else:
            return ''

    def _synchronize_tags(self, tags_data, hierarchy, table_scope='', ident=''):
        ...
        for tag_data in tags_data:
            ...
            tag_name = table_scope.split('_')[-1].lower()
            self._sync_file.write('{ident}<{tag_name}{properties}{immediate_close}>\n'.format(
                ...
                immediate_close=' /' if not hierarchy else ''))
            # synchronize this tag's children
            if hierarchy:
                for child_tag, child_hierarchy in hierarchy.items():
                    child_table_name = table_scope + '_' + child_tag.upper()
                    ...
                    self._synchronize_tags(chidren_data.fetchall(), child_hierarchy,
                        table_scope=child_table_name,
                        ident=ident + '  ')
                # close parent tag
